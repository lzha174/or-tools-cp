...
def half_lab_model(paras, job_data, day_index=0, period = morning_str):
    day_start = {morning_str:8, lunch_str: 14}
    day_end = {morning_str: 14, lunch_str: 21}
    ...
    starts_time = day_start[period]* seconds_per_hour + (day_index ) * day_in_seconds
    ends_time =  day_end[period] * seconds_per_hour + (day_index ) * day_in_seconds
    ...
    for case, tasks in job_data.items():
        ...
        for idx, task in enumerate(tasks):
            ...
            if (stage == paras[batch_stage_idx_str]):
                if priority == paras[nine_hour_priority_idx_str] and period == morning_str:
    ...
    if objective_choice == 'minimise_total_case_duration':
        ...
        for case, tasks in job_data.items():
            ...
            for idx in range(last_added_idx[j] + 1):
                ...
                reward = day_in_seconds * 8 - ready_times[j]
                reward_for_this_task = model.NewIntVar(0, day_in_seconds * 8, 'total duration {} {}'.format(j, idx))
                model.Add(reward_for_this_task== l_presences[j,idx] * reward)
