...
def main(args):
    ...
    # 2 machines can perform these jobs
    num_machines = 2
    ...
    horizon = 0
    for job in jobs:
        # only one task for each job
        for task in job:
            max_task_duration = 0
            # two alternatives for each job
    ...
    for job_id in all_jobs:
        ...
        for task_id in range(num_tasks):
            task = job[task_id]
            # one task, take the duration from the first alternatives
            min_duration = task[0][0]
            max_duration = task[0][0]
            print('min duration', min_duration, 'max duration', max_duration)
            # the first parameter is the duration
            num_alternatives = len(task)
            print('alternative', num_alternatives)
            # two alternatives for each task. only one task
            ...
            # this will get the actual min and max duratoin from all alternatives
            ...
            for alt_id in all_alternatives:
                ...
                # we have a rank variable to check the order of this task of the job
            ...
            # Only one machine can process each lot/ task .
    ...
    for machine_id in all_machines:
        ...
        for i in all_machine_tasks:
            ...
            # the starts time for this task is also zero for this task if it is the first one
            model.Add(machine_starts[i] == 0).OnlyEnforceIf(start_lit)

            model.AddImplication(start_lit, machine_presences[i])
            # Final arc from an arc to the dummy node.
            end_lit = model.NewBoolVar('')
            arcs.append([i + 1, 0, end_lit])
            model.AddImplication(end_lit, machine_presences[i])
            # Self arc if the task is not performed.
            # ass a self arc if this node is not included in the circu
            # a list of arcs. An arc is a tuple (source_node, destination_node, literal). The arc is selected in the circuit if the literal is true
            # . In case a node 'i' is not in the path, then there must be a loop arc 'i -> i' associated with a true literal. Otherwise this constraint will fail
            # so when presece is false, Not is True, meaning i->i is active, means this node is not in the path
            arcs.append([i + 1, i + 1, machine_presences[i].Not()])
            # set rank = -1 if not presence
            ...
            for j in all_machine_tasks:
                ...
                # if j follows i, both i and j on this machine
                ...
                if machine_resources[i] != machine_resources[j]:
                    # if using different resource, append this literal
                    transition_time = 3
                ...
                model.Add(machine_starts[j] >= machine_ends[i] +
                          transition_time).OnlyEnforceIf(lit)
                # model.Add(machine_starts[i] >= machine_ends[j] +
                #          transition_time).OnlyEnforceIf(lit.Not())
        if arcs:
